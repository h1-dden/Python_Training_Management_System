...
def clean_data():
    ...
    [employees,employee_skill,python_training,training_schedule]= get_data.csv_to_dataframe()
    ...
    clean_python_training['Assessment_Date'] = pd.to_datetime(clean_python_training['Assessment_Date'], format= '%d/%m/%Y')
    print(clean_python_training)
    ...
    for column in clean_training_schedule.columns[1:3]:  
        clean_training_schedule[f'{column}'] = pd.to_datetime(clean_training_schedule[f'{column}'], format= '%d/%m/%Y')
