...
import pandas as pd
from services import clean_data,database_connection

def upload_employees(employees,employee_skill,connection):
        
        """ Upload employee specific data into database"""
        #Stack Separation
        query = "SELECT * FROM Skills"
        df_skills = pd.read_sql(query, connection)
 
        employee_skill_new= pd.merge(employee_skill,
                                     df_skills, on = ['Skill_Name'],
                                     how= 'inner'
                                     )
        ...
        for index, row in counts.iterrows():
            if row['WebDev_Count'] < 3 and row['Data_Count'] >= 3:
                ...
                if row['Data_Count'] >= 3 and row['Data_Count'] <= 4:
                    skill_level_data.append({'Emp_ID': row['Emp_ID'], 
                                             'Stack': 'Data', 
                                             'Skill_Level': "L1"
                                             })
...
def upload_python_training(python_training,connection):

    """Upload python training data into database"""
    # Collect Emp_ID and Assessment_Date pairs from the DataFrame
    emp_assessment_pairs = list(zip(python_training.iloc[:, 1], 
                                    python_training.iloc[:, 2], 
                                    python_training.iloc[:, -2]
                                ))
...
def upload_employee_skills(employee_skill,connection):

    """Upload employee skills into database"""
...
def upload_training_schedule(training_schedule,connection):

    """Upload training schedule in database"""
...
def data_upload():
    ...
    """ Upload CSV files into database"""

    #Establish connection
    connection = database_connection.create_connection()

    #Unpack cleaned dataframes from list
    [employees,employee_skill,python_training,training_schedule]= clean_data.clean_data()

    #Call individual functions to upload data into database
    upload_employees(employees,employee_skill,connection)
    upload_employee_skills(employee_skill,connection)
    upload_python_training(python_training,connection)
    upload_training_schedule(training_schedule,connection)
